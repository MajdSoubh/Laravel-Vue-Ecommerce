<template>
  <table class="w-full bg-white rounded-xl">
    <thead class="">
      <tr>
        <th class="cursor-pointer" v-for="(key, ind) in attributes" :key="ind">
          <div
    ...
    <tbody class="text-center">
      ...
      <tr
        ...
        :data-id="item.id"
      >
        <td v-for="(key, ind) in attributes" :key="ind" class="">
          <router-link :to="{ name: 'order', params: { id: item.id } }">
            <div v-if="key?.component?.key">
              <component :is="key.component.key" :data="item"></component>
            </div>
            <div
              v-else
              class="max-w-[500px] overflow-hidden text-ellipsis line-clamp-2"
            >
              {{
                key.handle ? key.handle(item[key.name]) : access(key.name, item)
              }}
            </div>
          </router-link>
        </td>
